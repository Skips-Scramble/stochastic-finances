...
print(
    f"Test 1 - Savings override: {global_rates_dict['base_rf_interest_per_yr']} (expected 3.5)"
)
...
print(
    f"Test 2 - No savings override: {global_rates_dict['base_rf_interest_per_yr']} (expected 2.0)"
)
...
for i, ret in enumerate(retirement_list):
    if ret["interest_rate_per_yr"] is not None:
        print(
            f"Test 3.{i+1} - Retirement account {i}: override = {ret['interest_rate_per_yr']}"
        )
    else:
        print(
            f"Test 3.{i+1} - Retirement account {i}: will use global rate {global_rates_dict['base_mkt_interest_per_yr']}"
        )
